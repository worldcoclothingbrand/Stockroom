...
const AUTH_KEY    = "stockroom.auth.v1";
const PW_HASH     = "413b6e6ad790a8e00d1589ac320c9121b07b396831ad338c8f6e08bec3a25fc7";
...
function isAuthed() { return localStorage.getItem(AUTH_KEY) === PW_HASH; }
function doSignOut() { localStorage.removeItem(AUTH_KEY); location.reload(); }
...
function showLogin(err) {
  document.body.innerHTML =
    '<div class="login-screen"><div class="login-card">' +
      ...
      '<p class="login-sub">Enter your password</p>' +
      (err ? '<p class="login-error">' + err + '</p>' : '') +
      '<input id="pw" type="password" class="input" placeholder="Password" style="margin-top:12px;" autocomplete="current-password"/>' +
      '<button id="pw-btn" class="login-btn" style="margin-top:10px;">Sign in</button>' +
    '</div></div>';
  ...
  function attempt() {
    var val = inp.value;
    if (!val) return;
    hashPw(val).then(function(h) {
      if (h === PW_HASH) { localStorage.setItem(AUTH_KEY, PW_HASH); location.reload(); }
