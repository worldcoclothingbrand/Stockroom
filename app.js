...
app.innerHTML = `<button class="hamburger" id="hamburger-btn" aria-label="Open menu"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M3 6h18M3 12h18M3 18h18"/></svg> </button> <div class="sidebar-overlay" id="sidebar-overlay"></div> <div class="shell"> <aside class="sidebar" id="sidebar"> <div class="brand"> <div class="mark">S</div> <div> <h1>Stockroom</h1> <p>Inventory, barcodes, sales</p> </div> </div> <nav class="nav"> ${navButton("inventory", "box", "Inventory")} ${navButton("sell", "cart", "Sell")} ${navButton("labels", "barcode", "Labels")} ${navButton("reports", "chart", "Reports")} </nav> <div class="side-card"> <p class="side-note">Local-first app. Your products and sales are stored on this computer in this browser.</p> </div> </aside> <main class="main"> <div class="topbar"> <div class="title-block"> <h2>${pageTitle()}</h2> <p>${pageSubtitle()}</p> </div> <div class="actions"> <button class="ghost-button" data-action="export">${icon("download")}Export JSON</button> <button class="primary-button" data-action="new-product">${icon("plus")}New product</button> </div> </div> ${renderStats(t)} ${state.tab === "inventory" ? renderInventory(visible) : ""} ${state.tab === "sell" ? renderSell(visible) : ""} ${state.tab === "labels" ? renderLabels(visible) : ""} ${state.tab === "reports" ? renderReports(t) : ""} </main> </div> <div id="print-area"></div>`;
bindEvents();
}
...
return `<section class="grid stats"> <div class="stat"><small>Retail inventory value</small><strong>${money(t.retail)}</strong><span>${money(t.margin)} projected gross margin</span></div> <div class="stat"><small>Units in stock</small><strong>${number(t.units)}</strong><span>${state.products.length} products tracked</span></div> <div class="stat"><small>Low stock</small><strong>${number(t.low)}</strong><span>At or below reorder level</span></div> <div class="stat"><small>Total sales recorded</small><strong>${money(t.revenue)}</strong><span>${state.sales.length} completed checkouts</span></div> </section>`;
}

function renderFilters() {
return `<div class="search-row"> <input class="input" data-input="query" value="${escapeHtml(state.query)}" placeholder="Search products, SKU, barcode" /> <select class="select" data-input="category"> ${categories().map((c) =>`<option ${state.category === c ? “selected” : “”}>${escapeHtml(c)}</option>`).join("")} </select> <select class="select" data-input="status"> ${["All", "In stock", "Low", "Out"].map((s) => `<option ${state.status === s ? “selected” : “”}>${s}</option>`).join("")} </select> </div> `;
}

function renderInventory(products) {
return `<section class="panel"> <div class="panel-head"> <div><h3>Products</h3><p>${products.length} shown from ${state.products.length} total</p></div> </div> ${renderFilters()} ${renderProductTable(products)} </section>`;
}

function renderProductTable(products) {
if (!products.length) return `<div class="empty">No matching products yet.</div>`;
return `<table> <thead><tr><th>Product</th><th>Category</th><th>Stock</th><th>Price</th><th>Value</th><th>Barcode</th><th></th></tr></thead> <tbody> ${products.map((p) => { const status = productStatus(p); return`
<tr>
<td>
...
return `<section class="workspace"> <div class="panel"> <div class="panel-head"><div><h3>Add items</h3><p>Use search or barcode text to find products.</p></div></div> ${renderFilters()} ${renderProductTable(products)} </div> <aside class="panel"> <div class="panel-head"><div><h3>Current sale</h3><p>${cartItems.length} line items</p></div></div> <div class="checkout-list"> ${cartItems.length ? cartItems.map(({ product, qty }) => checkoutItem(product, qty)).join("") :`<div class="empty">Add products to start a sale.</div>`} </div> <div class="cart-total"> <div class="total-row"><span>Subtotal</span><strong>${money(subtotal)}</strong></div> <div class="total-row"><span>Estimated margin</span><span>${money(subtotal - cost)}</span></div> <button class="primary-button" data-action="complete-sale" ${cartItems.length ? "" : "disabled"}>${icon("cart")}Complete sale</button> <button class="ghost-button" data-action="clear-cart">Clear</button> </div> </aside> </section> `;
}

function checkoutItem(product, qty) {
return `<div class="mini-card checkout-item"> <div><div class="product-name">${escapeHtml(product.name)}</div><span class="sku">${escapeHtml(product.sku)} · ${money(product.price)}</span></div> <div class="stepper"> <button data-action="cart-minus" data-id="${product.id}">${icon("minus")}</button> <strong>${qty}</strong> <button data-action="cart-add" data-id="${product.id}">${icon("plus")}</button> </div> </div>`;
}

function renderLabels(products) {
const selected = state.products.filter((p) => state.labelIds.has(p.id));
return `<section class="workspace"> <div class="panel"> <div class="panel-head"> <div><h3>Choose labels</h3><p>Select products, then print a sheet.</p></div> <button class="ghost-button" data-action="select-visible">${icon("plus")}Select visible</button> </div> ${renderFilters()} ${renderProductTable(products)} </div> <aside class="panel"> <div class="panel-head"> <div><h3>Preview</h3><p>${selected.length} products selected</p></div> <button class="primary-button" data-action="print-labels" ${selected.length ? "" : "disabled"}>${icon("print")}Print</button> </div> <div class="label-list"> <div class="label-preview"> ${selected.length ? selected.slice(0, 8).map(labelMarkup).join("") :`<div class="empty">No labels selected.</div>`} </div> </div> </aside> </section> `;
}

function labelMarkup(product) {
return `<div class="print-label"> <strong>${escapeHtml(product.name)}</strong> ${barcodeSvg(product.barcode, { height: 54, module: 1 })} <small>${escapeHtml(product.sku)} · ${money(product.price)}</small> </div>`;
}
...
<tbody>
${topProducts.map((p) => `<tr> <td><div class="product-cell"><span class="swatch" style="${p.image ?`background:url(’${p.image}’) center/cover no-repeat` : "background:#333"}">${p.image ? "" : ""}</span><div><span class="product-name">${escapeHtml(p.name)}</span><span class="sku">${escapeHtml(p.sku)}</span></div></div></td> <td>${number(p.stock)}</td> <td>${money(p.stock * p.price)}</td> <td>${money(p.stock * p.cost)}</td> </tr>`).join(””)}
</tbody>
</table>
</div>
<aside class="drawer">
...
modal.innerHTML = `<form class="modal" id="product-form"> <div class="panel-head"><div><h3>${existing ? "Edit product" : "New product"}</h3><p>Barcode is generated automatically, but you can replace it.</p></div></div> <div class="modal-body"> <div class="form-grid"> ${field("Name", "name", product.name, "text", true)} ${field("SKU", "sku", product.sku, "text", true)} ${field("Category", "category", product.category, "text", true)} ${field("Barcode", "barcode", product.barcode, "text", true)} ${field("Cost", "cost", product.cost, "number", true)} ${field("Price", "price", product.price, "number", true)} ${field("Stock", "stock", product.stock, "number", true)} ${field("Reorder level", "reorder", product.reorder, "number", true)} <div class="field full"> <label>Product Image</label> <div class="image-upload-area" id="image-upload-area"> ${product.image ?`<img src="${product.image}" class="image-preview" id="image-preview" alt="Product"/>`:`<div class="image-placeholder" id="image-placeholder">
...
</div>`} </div> <input type="file" id="image-file-input" accept="image/*" style="display:none"/> <input type="hidden" name="image" id="image-hidden" value="${escapeHtml(product.image || "")}"/> ${product.image ?`<button type="button" class="ghost-button" id="remove-image-btn" style="margin-top:8px; width:100%;">Remove image</button>`: ""} </div> <div class="field full"><label>Notes</label><textarea class="textarea" name="notes">${escapeHtml(product.notes)}</textarea></div> <div class="field full"><label>Preview</label><div class="barcode-wrap">${barcodeSvg(product.barcode)}</div></div> </div> </div> <div class="modal-foot"> <button type="button" class="ghost-button" data-close>Cancel</button> <button type="submit" class="primary-button">Save product</button> </div> </form>`;
...
removeBtn.onclick = () => {
hiddenInput.value = “”;
uploadArea.innerHTML = `<div class="image-placeholder" id="image-placeholder"> <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg> <span>Click or drag to upload image</span> <small>PNG, JPG, WEBP supported</small> </div>`;
removeBtn.remove();
};
}
...
hiddenInput.value = “”;
uploadArea.innerHTML = `<div class="image-placeholder"> <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg> <span>Click or drag to upload image</span> <small>PNG, JPG, WEBP supported</small> </div>`;
